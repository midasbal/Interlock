...
**Finding 10: a single idempotency-keyed execute call is sometimes broadcast twice through two different execution paths, and the execution KeeperHub itself was tracking is sometimes falsely reported as failed. Confirmed as a recurring pattern, not a one-off, by the post-execution reconciler built for roadmap item 1.**
